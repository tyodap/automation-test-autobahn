...
const externalScanName = fileUpload.generateUploadFileName("Invicti-external");
const internalScanName = fileUpload.generateUploadFileName("Invicti-internal");
...
beforeEach(() => {
  ...
  fileUpload.selectOriginAndType("New", "Invicti");

  fileUpload.scanType.click();
});

describe.skip("Upload New Invicti", { tags: ["@daily"] }, () => {
  it("Should be able to upload new external Invicti file", () => {
    ...
    fileUpload.uploadFile("cypress/fixtures/data/invicti.json");
  ...
  it("Should be able to upload new internal Invicti file", () => {
    fileUpload.scanTypeDropdown
      .should("be.visible")
      .and("not.have.class", "ant-dropdown-hidden")
      .within(() => {
