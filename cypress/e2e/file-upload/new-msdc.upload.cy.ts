...
const externalScanName = fileUpload.generateUploadFileName("MSDC-external");
const internalScanName = fileUpload.generateUploadFileName("MSDC-internal");
...
beforeEach(() => {
  ...
  fileUpload.selectOriginAndType("New", "MS Defender for Cloud");

  fileUpload.scanType.click();
});

describe.skip("Upload New MSDC", { tags: ["@daily"] }, () => {
  it("Should be able to upload new external MSDC file", () => {
    ...
    fileUpload.uploadFile("cypress/fixtures/data/msdc.csv");
  ...
  it("Should be able to upload new internal MSDC file", () => {
    fileUpload.scanTypeDropdown
      .should("be.visible")
      .and("not.have.class", "ant-dropdown-hidden")
      .within(() => {
